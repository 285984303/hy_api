<?php namespace App\Library\Easemob;
...
class easemob
{
     ...
     * @param bool   $param_use_array 知否强制 $request_body为array
     *
     * @return array
     */
	...
	// 授权注册
	function user_register_authorization($username, $password, $nickname = ""){
        $url = "users";
        $http_method = "POST";
     ...
     * @return bool | string 错误直接返回false
     */
...
//        echo $files;
    ...
    function reset_password($username,$newpassword){
        ...
        while($return===false){
            $result = $this->api_request($path, json_encode($request_body,JSON_UNESCAPED_UNICODE) , 'PUT', $header);
            if(empty($result[0]) || $result[0] != 200 ){
                throw new \Exception('easemob code:'.$result[0]??"not ");
            }
            $return = $this->result_handler($result);
        }
