...
use App\Models\Appointment\AppointmentTcpPhoto;
...
{
    // 终端注册
    static public function message_0100($header, $body_hex, $line)
    {
        $body = [
...
//            }
            $termInfoByImei = TimingTerm::where('imei', $term->imei)->count();
            $termInfoBySim = TimingTerm::where('sim', $term->sim)->count();
            $termInfoBySn = TimingTerm::where('sn', $term->sn)->count();

            if ($termInfoByImei || $termInfoBySim || $termInfoBySn) {
                throw new \Exception('终端已被注册', 3);
            }
            $term->getUniqueId();
            $term->putRecord();
            if ($body['model'] == 'SDJ-XK1700-YA') {//计时终端需要绑定
            ...
            $redis->lpush('tcp_msg_send', $line);
            $redis->hmset('refoward:' . $header['message_id'] . str_pad($header['message_no'], 4, '0', STR_PAD_LEFT), ['message' => $line, 'time' => time(), 'num' => 0]);
    ...
    // 心跳
    static public function message_0002($header, $body_hex, $line)
    {
    ...
    // 终端注销
    static public function message_0003($header, $body_hex, $line)
    {
        $term = TimingTerm::where('sim', $header['mobile'])->first();
        if ($term) {
            // todo 注销终端
            $result = $term->writeOff();//删除计时终端
//            $term->delete();//@todo 没有该方法
            if ($result->error()) {
                throw new \Exception($result->message);
            }
            $vehicle = Vehicle::find($term->vehicle_id);
            $term->remove_blind_record($vehicle->vehicle_numbers);
            TimingTerm::find($term->id)->delete();
        }
    ...
    // 通用
    static public function message_8001($header, $body_hex, $line)
    {
        $message_body = [
            Bin::word($header['message_no']),
            hex2bin($header['message_id']),
    ...
    // 通用
    static public function message_failed($header, $body_hex, $line)
    {
        $message_body = [
            Bin::word($header['message_no']),
            hex2bin($header['message_id']),
    ...
    // 通用
    static public function message_error($header, $body_hex, $line)
    {
        $message_body = [
            Bin::word($header['message_no']),
            hex2bin($header['message_id']),
    ...
    // 通用
    static public function message_unsupposed($header, $body_hex, $line)
    {
        $message_body = [
            Bin::word($header['message_no']),
            hex2bin($header['message_id']),
    ...
    // 终端鉴权
    static public function message_0102($header, $body_hex, $line)
    {
        $body = [
    ...
    // 查询终端参数应答
    static public function message_0104($header, $body_hex, $line)
    {
        $body = [
    ...
    // 位置信息汇报
    static public function message_0200($header, $body_hex, $line)
    {
        $body = [
    ...
    // 位置信息查询应答
    static public function message_0201($header, $body_hex, $line)
    {
        $body = [
            '报警标识' => hexdec(substr($body_hex, 0, 8)),
            '状态' => hexdec(substr($body_hex, 8, 8)),
    ...
    // 数据上行透传
    static public function message_0900($header, $body_hex, $line)
    {
        ...
        return self::$function($header, $body_hex, $line);
    }

    // 数据下行透传@todo[]
    static public function message_8900($header, $body_hex, $line)
    {
    }

    // 平台登录
    static public function message_01f0($header, $body_hex, $line)
    {
    ...
    // 平台登出
    static public function message_01f1($header, $body_hex, $line)
    {
    ...
    // 设置终端参数
    static public function message_8103($header, $body_hex, $line)
    {
    ...
    // 教练登录
    static public function message_cross_0101($header, $body_hex, $line)
    {
...
//        $admin->signIn();
        $appointment = Appointment::where('admin_id', $admin->id)
            ->where('start_time', '<=', date('H:i:s'))
            ->where('finish_time', '>', date('H:i:s'))
            ->where('date', date('Y-m-d'))
            ->whereIn('status', [Appointment::STATUS_TAKEN, Appointment::STATUS_DOING])
            ->first();
        if (!$appointment->admin_sign_in_time) {
            $appointment->admin_sign_in_time = date('H:i:s');
            $appointment->save();
        }
    ...
    // 教练登出
    static public function message_cross_0102($header, $body_hex, $line)
    {
        ...
        try {
            $appointment = Appointment::where('admin_id', $admin->id)
                ...
                ->where('finish_time', '>', date('H:i:s'))
                ->where('date', date('Y-m-d'))
                ->whereIn('status', [Appointment::STATUS_DONE, Appointment::STATUS_DOING])
                ->first();
            if ($appointment) {
                $appointment->admin_sign_out_time = date('H:i:s');
                $appointment->save();
            }
        } catch (\Exception $e) {
            \Log::debug('教练签退error:' . $e->getMessage());
        }
        $message_body = [
            Bin::byte(1),
            Bin::byte_array($body['教练员编号'], 16),
    ...
    // 学员登录
    static public function message_cross_0201($header, $body_hex, $line)
    {
        ...
        try {
            $appointment = Appointment::where('user_id', $user->id)
                ...
                ->first();
            //获取终端
            if ($appointment->model == 1) {
                $appointment->sign_in();//计时
            } else {
                $appointment->simulation_sign_in();//模拟
            }
        } catch (\Exception $e) {
            \Log::debug('簽到error:' . $e->getMessage());
            $message_body = [
                Bin::byte(9),
            ...
            return self::answer_cross('8201', $header, $message_body);
        }
        $schoolOutline = \DB::table('school_licence_outline')->where('licence_id', $appointment->vehicle->licence_type_id)->first();
        //科目三判断
//        if($user->subject_2>0 && $user->subject_3==0 ) {
//
//        } else {
//
//        }
        if ($schoolOutline) {
            $totalTime = $schoolOutline->first_time + $schoolOutline->sece_time + $schoolOutline->sec_time + $schoolOutline->thr_time + $schoolOutline->three_time + $schoolOutline->forth_time;
            $doneMins = Appointment::where('user_id', $user->id)->pluck('min')->sum();
            $doneDistance = Appointment::where('user_id', $user->id)->pluck('distance')->sum();
            $message_body = [
                Bin::byte(1),
                Bin::byte_array($body['学员编号'], 16),
                Bin::word($totalTime), // 总培训学时 * 1 min
                Bin::word($doneMins), // 当前培训部分已完成学时
                Bin::word($schoolOutline->mil_nums), // 总培训里程 * 0.1 km
                Bin::word($doneDistance), // 当前培训部分已完成里程
                Bin::byte(0), // 是否报读附加消息
                Bin::byte(0), // 附加消息长度
            ];
        } else {
    ...
    // 学员登出
    static public function message_cross_0202($header, $body_hex, $line)
    {
        $body = [
            '学员编号' => implode(unpack('a*', hex2bin(substr($body_hex, 0, 32)))),
            ...
            '课堂ID' => hexdec(substr($body_hex, 52, 8)),
            [
...
//        \Log::debug('body:',json_encode($body));
        ...
        $user = User::where('user_numbers', $body['学员编号'])->first();
        $appointment = Appointment::where('user_id', $user->id)
//            ->where('date', date('Y-m-d'))//只根据课程id获取信息
            ->where('id', $body['课堂ID'])
            ->whereIn('status', [Appointment::STATUS_TAKEN, Appointment::STATUS_DOING])
            ->orderBy('id', 'desc')
            ->first();
        $data = [];
        $data['history'] = ['distance' => $body['该次登录总里程'] / 10, 'totalTime' => $body['该次登录总时间']];

        try {
            $appointment->sign_out(json_encode($data));

        } catch (\Exception $e) {
            \Log::debug('学员签退error:' . $e->getMessage());
        }
    ...
    // 上报学时记录
    static public function message_cross_0203($header, $body_hex, $line)
    {
        ...
        $body = [
            ...
            '课堂ID' => hexdec(substr($body_hex, 118, 8)),
        ...
        // todo 更新学时记录 及里程
        try {
            Appointment::find($body['课堂ID'])->increment('min', 1);
            Appointment::find($body['课堂ID'])->increment('distance', $body['里程'] / 10);
...
//            }
            //记录上报轨迹
            $log_data = [
                'appointment_id' => $body['课堂ID'],
                'user_numbers' => $body['学员编号'],
                'rnum' => $body['学时记录编号'],
                'type' => $body['上报类型'],
                'coach' => $body['教练'],
                'course' => $body['培训课程'],
                ...
                'position' => json_encode([
                    'identification' => $body['位置信息']['报警标识'],
                    'status' => $body['位置信息']['状态'],
                    'dimension' => $body['位置信息']['维度'],
                    ' longitude ' => $body['位置信息']['经度'],
                    'travel_speed' => $body['位置信息']['行驶记录速度'],
                    'location_speed' => $body['位置信息']['卫星定位速度'],
                    'direction' => $body['位置信息']['方向'],
                    'time' => $body['位置信息']['时间'],
                    'additional' => $body['位置信息']['附加信息'],
                ]),
                'hours' => $body['记录产生时间'],
                'create_time' => time(),
                'update_time' => time()
            ...
            \DB::table('hours_log')->insert($log_data);

        } catch (\Exception $e) {
            \Log::debug('更新学时记录error:' . $e->getMessage());
        }
        ...
        self::message_8001($header, null, null);
    }

    // 命令上报学时应答
    static public function message_cross_0205($header, $body_hex, $line)
    {
    ...
    // 立即拍照应答
    static public function message_cross_0301($header, $body_hex, $line)
    {
    ...
    // 查询照片应答
    static public function message_cross_0302($header, $body_hex, $line)
    {
    ...
    // 查询照片结果
    static public function message_cross_0303($header, $body_hex, $line)
    {
    ...
    // 上传指定照片应答
    static public function message_cross_0304($header, $body_hex, $line)
    {
    ...
    // 上传照片初始化
    static public function message_cross_0305($header, $body_hex, $line)
    {
        ...
        try {
            if (AppointmentTcpPhoto::checkTcpPhotoExists($body['照片编号']) > 0) {
//                throw new \Exception('照片编号重复或错误', 1);
                $messageCode = 1;
            }
        } catch (\Exception $e) {
            $messageCode = 9;
            \Log::debug('其他错误:' . $e->getMessage());
        }
...
//        $message_body = Bin::byte(0);
        $message_body = Bin::byte($messageCode);

        if ($messageCode == 0) {
            $data['photo_number'] = $body['照片编号'];
            $data['appointment_id'] = $body['课堂ID'];
            $appointment = Appointment::find($body['课堂ID']);
            $data['school_id'] = $appointment->school_id;
            $data['photo_type'] = $body['发起图片的事件类型'];
            $data['photo_url'] = \Config::get('oss.CONFIG.HOST') . '/' . $appointment->school_id . '/' . \Config::get('oss.tcp.train') . date("Y-m-d") . $body['照片编号'] . '.jpg';
            \DB::table('appointment_tcp_photo')->insert($data);
        }
    ...
    // 上传照片数据包
    static public function message_cross_0306($header, $body_hex, $line)
    {
        try {
            ...
            //圖片存放地址
            $imgSrc = public_path('upload') . '/tcp';
            $fileName = $num . '.jpg';
            $file = $imgSrc . '/' . $fileName;
            if (!file_exists($file)) {
                @touch($file);
            }
//            file_put_contents($file, $image, FILE_APPEND);//保存到服务器
            //上传至阿里云-地址加上日期目录
//            $path = \Config::get('oss.tcp.train');
            $tcpPhotoInfo = AppointmentTcpPhoto::getSchoolIdByPhotoNum($num);
            $school_id = $tcpPhotoInfo->school_id;
            $path = $school_id . '/' . \Config::get('oss.tcp.train') . date("Y-m-d") . '/';
            OSS::publicUploadContent(\Config::get('oss.CONFIG.BUCKET'), $path . $fileName, $image);

            \Log::debug('图片上传成功');
            //更新图片上传状态
            \DB::table('appointment_tcp_photo')->where(['photo_number' => $num])->update(['up_status' => 1]);
        } catch (\Exception $e) {

            file_put_contents('/tmp/upload.log', $e->getMessage() . '==\r\n', FILE_APPEND);
            \Log::debug('图片上传error:' . $e->getMessage());
        }
    ...
    // 设置计时终端应用参数应答
    static public function message_cross_0501($header, $body_hex, $line)
    {
    ...
    // 设置禁训状态应答
    static public function message_cross_0502($header, $body_hex, $line)
    {
    ...
    // 设置计时终端应用参数应答
    static public function message_cross_0503($header, $body_hex, $line)
    {
    ...
    // 身份认证
    static public function message_cross_0401($header, $body_hex, $line)
    {
    ...
    // 统一编号
    static public function message_cross_0402($header, $body_hex, $line)
    {
    ...
    // 车辆绑定信息
    static public function message_cross_0403($header, $body_hex, $line)
    {
    ...
    // message template
    static public function message($header, $body_hex, $line)
    {
