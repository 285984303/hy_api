				...
				downloadDrive : function(){
					...
					messageBox({messageType: "confirm", text: "请安装相关硬件驱动！点击确定下载驱动。", 
						callback: function(result){
							if(result)
							{
								window.location.href = "/admin/statistics/drive/download";
							}
							closeMessage();
					}});
				}
...
function setCertificateData(result)
{
	...
	$('input[name="id_card"]').val(result.Certificate.IDNumber);
    ...
    $('input[name="user_truename"]').val(result.Certificate.Name);
	// $("#gender").val(result.Certificate.Sex);
    if (result.Certificate.Sex == '男') {
        $('form').find('input[name="user_sex"]').eq(0).attr('checked','checked');
    } else {
        $('form').find('input[name="user_sex"]').eq(1).attr('checked','checked');
    }
	// $("#nation").val(result.Certificate.Nation);
	// $("#address").val(result.Certificate.Address);
	$('input[name="id_card_address"]').val(result.Certificate.Address);
	$('input[name="nation"]').val(result.Certificate.Nation);
	$('input[name="birthday"]').val(result.Certificate.Birthday.replace(/\./g,"-").substr(0,10));
}
